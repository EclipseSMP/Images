...
if [ "${FORGE_COMPATIBILITY}" = 1 ] && [ -z "${JLINE_ARGS}" ] && [ -z "${FORGE_VERSION}" ]; then
    ...
    echo -e "\033[1;35mNOTE: \033[0mForge compatibility mode is enabled."
fi
...
if [ "${LOG4J2_VULN_WORKAROUND}" = 1 ] && [ -z "${FORGE_VERSION}" ]; then
    ...
    echo -e "\033[1;35mNOTE: \033[0mThe Log4j2 vulnerability workaround has been enabled. If you're running an unpatched server software 
build, remember to update ASAP as this workaround may be removed at any time, and is not effective in older versions of the game."
fi
...
if [ "${SIMD_OPERATIONS}" = 1 ] && [ -z "${FORGE_VERSION}" ]; then
    ...
    echo -e "\033[1;35mNOTE: \033[0mSIMD operations are enabled."
fi
...
if [ "${AIKAR_FLAGS}" = 1 ]; then
    ...
    echo -e "\033[1;35mNOTE: \033[0mEnabled Aikar's Flags"
fi

# Print startup command to console
echo -e "\033[1;35mConsole@EclipseSMP:~\$\033[1;37m ${MODIFIED_STARTUP}"
